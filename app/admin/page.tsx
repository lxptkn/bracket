...
export default function AdminPage() {
  ...
  if (!session) return (
    <div className="min-h-screen bg-stone-800 text-stone-200 p-6">
      <button
        className="border border-stone-700 bg-stone-900 text-stone-200 px-4 py-2"
        onClick={() => signIn()}
      >
        Sign in as Admin
      </button>
  ...
  return (
    <div className="min-h-screen bg-stone-800 text-stone-200">
      ...
      <div className="flex items-center justify-end gap-3">
        <button
          className="border border-stone-700 bg-stone-900 text-stone-200 px-3 py-1"
      ...
      <div className="bg-stone-900 border border-stone-700 p-4">
        <h2 className="text-xl font-semibold mb-2">Seasons</h2>
        <div className="flex gap-2 items-center mb-4">
          <input
            className="border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1"
            placeholder="Add new season (e.g. 2024 or June 2025)"
            value={newSeason}
            onChange={(e) => setNewSeason(e.target.value)}
          />
          <button className="border border-stone-700 bg-stone-900 text-stone-200 px-3 py-1" onClick={createSeason}>Add</button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          {seasons.map((s) => (
            <div key={s} className="flex items-center justify-between border border-stone-700 bg-stone-800 px-3 py-2">
      ...
      {selectedSeason && (
        <div className="space-y-4 bg-stone-900 border border-stone-700 p-4">
          <h2 className="text-xl font-semibold">Participants</h2>
          <div className="flex flex-wrap gap-2 items-center">
            <input className="border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1" placeholder="Add new global participant" value={newParticipantName} onChange={(e)=>setNewParticipantName(e.target.value)} />
            <input className="border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1 w-28" placeholder="Seed (opt)" value={newParticipantSeed} onChange={(e)=>setNewParticipantSeed(e.target.value)} />
            <button className="border border-stone-700 bg-stone-900 text-stone-200 px-3 py-1" onClick={async ()=>{
            ...
            <div className="border border-stone-700 bg-stone-900 p-3">
              ...
              <div className="flex flex-wrap gap-2">
                {availableToAdd.map((p)=> (
                  <span key={p.name} className="inline-flex items-center gap-1 bg-stone-800 text-stone-200 px-3 py-1 text-sm border border-stone-700">
                    <span>{p.name}</span>
                ...
                {availableToAdd.length === 0 && (
                  <span className="text-sm text-stone-400">All global participants are already in this season.</span>
                )}
              </div>
            </div>
            ...
            <div className="border border-stone-700 bg-stone-900 p-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {participants.length === 0 && (
                  <span className="text-sm text-stone-400">No participants in this season yet.</span>
                )}
              </div>
            </div>
          </div>
      ...
      {selectedSeason && (
        <div className="space-y-4 bg-stone-900 border border-stone-700 p-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <input className="border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1" placeholder="Add new global moderator" value={newModeratorName} onChange={(e)=>setNewModeratorName(e.target.value)} />
            <button className="border border-stone-700 bg-stone-900 text-stone-200 px-3 py-1" onClick={async ()=>{
            ...
            <div className="border border-stone-700 bg-stone-900 p-3">
              ...
              <div className="flex flex-wrap gap-2">
                {globalModerators
                  ...
                  .map((m)=> (
                  <span key={m.name} className="inline-flex items-center gap-1 bg-stone-800 text-stone-200 px-3 py-1 text-sm border border-stone-700">
                    <span>{m.name}</span>
                    <button title="Add to this season" className="w-6 h-6 inline-flex items-center justify-center bg-emerald-700 text-white border border-emerald-600" onClick={async ()=>{
            ...
            <div className="border border-stone-700 bg-stone-900 p-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {seasonModerators.length === 0 && (
                  <span className="text-sm text-stone-400">No moderators in this season yet.</span>
                )}
              </div>
            </div>
          </div>
...
function RoundWinnersManager({ season, refreshSignal }: { season: string; refreshSignal?: number }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {rounds.map((round) => (
        <div key={round} className="border border-stone-700 bg-stone-900 p-3">
          <div className="font-semibold mb-2 text-stone-100">{round}</div>
          <div className="space-y-2">
            {(bracket[round] || []).map((m) => (
              <div key={`${round}-${m.matchNumber}`} className="flex items-center gap-2">
                <span className="w-20 text-sm text-stone-400">Match {m.matchNumber}</span>
                <button
                  disabled={!m.player1?.name}
                  className={`border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1 ${m.winner === m.player1?.name ? 'bg-emerald-900/30 border-emerald-500 text-emerald-300' : ''} ${!m.player1?.name ? 'opacity-50 cursor-not-allowed' : ''}`}
                ...
                <button
                  ...
                  className={`border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1 ${m.winner === m.player2?.name ? 'bg-emerald-900/30 border-emerald-500 text-emerald-300' : ''} ${!m.player2?.name ? 'opacity-50 cursor-not-allowed' : ''}`}
...
function MonthSelectors({ seasons, selectedSeason, onChangeSeason, season, month1, month2, onChange }: { seasons: string[]; selectedSeason: string; onChangeSeason: (s: string)=>void; season: string; month1: string; month2: string; onChange: (m1: string, m2: string) => void }) {
  // Build dropdown options: months Jan..Dec, years: current year and next year
  const now = new Date()
  ...
  return (
    <div className="space-y-3 bg-stone-900 border border-stone-700 p-4">
      <h3 className="font-semibold">Current Season</h3>
      <div className="flex flex-wrap items-center gap-3">
        <label className="text-sm">Editing:</label>
        <select className="border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1" value={selectedSeason} onChange={(e)=> onChangeSeason(e.target.value)}>
          {seasons.map((s)=> (
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select className="border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1" value={a.m} onChange={(e)=> onChange(f(a.y, Number(e.target.value)), f(a.y + (Number(e.target.value) === 11 ? 1 : 0), (Number(e.target.value) + 1) % 12))}>
          {months.map((m)=> (<option key={m.index} value={m.index}>{m.name}</option>))}
        </select>
        <select className="border border-stone-700 bg-stone-900 text-stone-200 px-2 py-1" value={a.y} onChange={(e)=> onChange(f(Number(e.target.value), a.m), f(Number(e.target.value) + (a.m === 11 ? 1 : 0), (a.m + 1) % 12))}>
        ...
        <button className="border border-stone-700 bg-stone-900 text-stone-200 px-3 py-1" onClick={async ()=>{
          ...
          alert('Saved season month(s)')
        }}>Save</button>
      </div>
      <div className="text-xs text-stone-500">Pick the first month; the second is auto-set to the next month.</div>
    </div>
  )
}
